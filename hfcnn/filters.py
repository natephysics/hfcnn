...
class Filter:
    def __init__(
        self, fn_req_imgs: bool = False, raw_img_folder: str = False, **kwargs
    ):
...
class Data_Selection_Filter(Filter):
    def __init__(self, raw_img_folder: str = False, int_threshold: float = 5, **kwargs):
        super().__init__(fn_req_imgs=True, raw_img_folder=raw_img_folder, **kwargs)
        self.raw_img_folder = raw_img_folder
...
class Missing_Data_Filter(Filter):
    def __init__(self, raw_img_folder: str = False, **kwargs):
        super().__init__(fn_req_imgs=True, raw_img_folder=raw_img_folder, **kwargs)
        self.raw_img_folder = raw_img_folder
...
def split(prog_num_list: list, ratio_list: Union[float, list]):
    ...
    if "PL_GLOBAL_SEED" in os.environ:
        random.seed(int(os.environ["PL_GLOBAL_SEED"]))
    ...
    return [
        prog_num_list[sub_index[i] : sub_index[i + 1]]
